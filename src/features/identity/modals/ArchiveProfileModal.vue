...
async function handleFormSubmission() {
 ...
 // 1. Tell the ecosystem the user has initiated an action
  ...
  startLoading()
  const { success, error } = await profileStore.archiveProfile(formData.value!)
...
<template>
  ...
  <input 
        ...
        id="Confirm"
      />
     <label For="Confirm">Tick to confirm archive action </label>
  ...
  <span v-if="formSubmitted && validationErrors.confirm" class="validation-message">
        ...
        <div class="button-holder">
          ...
          >
            {{ progressState.type === 'Loading' ? 'Submitting...' : 'Archive Profile' }}
          </button>
        </div>
    </form>
  </div>
