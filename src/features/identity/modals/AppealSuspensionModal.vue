...
import type { AppealSuspensionRequest } from '../types/IdentityTypes'
...
const formData = ref<AppealSuspensionRequest>({
      notes: '',
      confirm: false
})
...
const validationErrors = computed(() => {
  ...
  return {
    ...
    notes: notesText === '' || notesText.length < 10 || notesText.length > 1024
      ? 'Notes must be between 10 and 1024 characters'
      : '',
...
async function handleFormSubmission() {
 ...
 // 1. Tell the ecosystem the user has initiated an action
  ...
  startLoading()
  const { success, error } = await profileStore.appealSuspension(formData.value!)
...
<template>
  ...
  <span v-if="formSubmitted && validationErrors.confirm" class="validation-message">
        ...
        <div class="button-holder">
          ...
          >
            {{ progressState.type === 'Loading' ? 'Submitting...' : 'Appeal' }}
          </button>
        </div>
    </form>
  </div>
