...
import {type InsightDetailDto, type GetInsightDetailPersonalizeResponse, type GetInsightPageEnrichmentResponse, 
  initializeInsightDetailEngagement, hydrateInsightEngagement} from '../types/InsightsTypes.ts';

export const useInsightDetailStore = defineStore('insightDetail', () => {
...
async function loadInsight(insightId: string): Promise<{ success: boolean; error: APIError | null }> {
  ...
  try {
    ...
    const outcome = await getAsync<InsightDetailDto>(`api/insights/${insightId}`, false, {} as InsightDetailDto, feedController.signal);
...
// 🔄 Map and clean the data stream BEFORE it hits the UI state engine
      insight.value = initializeInsightDetailEngagement(outcome.value);
      insight.value.creator = initializeAccountEngagement(outcome.value.creator)
...
async function loadArchivedInsight(insightId: string): Promise<{ success: boolean; error: APIError | null }> {
  ...
  try {
    ...
    // Note: Assuming getAsync is part of your API client layer
    const outcome = await getAsync<InsightDetailDto>(`api/insights/archives/${insightId}`, true, {} as InsightDetailDto, feedController.signal);
...
// 🔄 Map and clean the data stream BEFORE it hits the UI state engine
  ...
  async function hydratePersonals() {
    ...
    for (let i = 0; i <= maxRetries; i++) {
      ...
      try {
        ...
        const outcome = await getAsync<GetInsightDetailPersonalizeResponse>(`api/insights/personalized/${insight.value?.insightId}/${insight.value?.creator.accountId}`, 
        true, {} as GetInsightDetailPersonalizeResponse, hydrateController.signal);

        if(outcome.isSuccess){

           if(outcome.value && insight.value != null){
           
            insight.value.engagement.isFavorite = outcome.value.isFavorite
            insight.value.engagement.hasFlagged = outcome.value.hasFlagged
            insight.value.engagement.myVote = outcome.value.myVote

            insight.value.creator.engagement.isFavorite = outcome.value.iscreatorFavorite
          }
  ...
  async function enrichInsight() {
      ...
      try {
        ...
        // Replace with your actual Axios/Fetch HTTP abstraction layout instance
        const outcome = await getAsync<GetInsightPageEnrichmentResponse>(`api/insights/enrichment/${insight.value?.insightId}`, 
         true, {} as GetInsightPageEnrichmentResponse, enrichController.signal);
        ...
        if(outcome.isSuccess){
           ...
           if(outcome.value && insight.value != null){
            ...
            hydrateInsightEngagement(insight.value.engagement, outcome.value.stats);
...
async function recordView() {
  ...
  try {
    ...
    const formData = {
      contentId: insight.value?.insightId
    }
  ...
  function activateEngagementButtons() {
    ...
    if(insight.value !== null){
        insight.value.engagement.isEngagementLoaded = true;
        insight.value.creator.engagement.isEngagementLoaded = true;
    }
