...
import type { UpdateSummaryRequest } from '../types/InsightsTypes'
...
const formData = ref<UpdateSummaryRequest>({
   insightId: '',
  summary: ''
})
...
onBeforeMount(() => {
 ...
 formData.value.summary = insightStore.activeInsight.summary ?? ''
 resetProgress()
...
async function handleFormSubmission() {
 ...
 const { success, error } = await insightStore.updateInsightSummary(formData.value!)
...
<template>
    ...
    <h2>This insight in a sentence</h2>
