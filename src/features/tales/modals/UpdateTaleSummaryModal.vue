...
import type { UpdateSummaryRequest } from '../types/TalesTypes'
...
onBeforeMount(() => {
 ...
 formData.value.summary = taleStore.activeTale.summary ?? ''
 resetProgress()
...
// 1. Tell the ecosystem the user has initiated an action
 ...
 const { success, error } = await taleStore.updateTaleSummary(formData.value!)
